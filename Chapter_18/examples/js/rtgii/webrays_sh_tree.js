...
function wrays_viewer_init() {
  ...
  WebRaysViewer.shader_urls = [
                               ...
                               'kernels/screen_fill.vs.glsl', 'kernels/rtgii/wrays_viewer_rtg_shadows.glsl',
    ...
    Promise.all(responses.map(res => res.blob())).then(buffers => {
      ...
      Promise.all(decoded_imgs.map(img=>img.decode())).then(responses => {
        ...
        Promise.all(WebRaysViewer.shader_urls.map(url =>
          ...
          const gltf_file = 'gltf/TreesShadows.gltf';
          webrays_load_gltf(gltf_file).then(res => {
            WebRaysViewer.mesh = res;
            {
              ...
              // custom camera
              if(gltf_file === 'gltf/TreesShadows.gltf')
              {
                center[0] = 1.6819782257080078 + 0.7654734253883362;
                center[1] = 3.816258668899536 - 0.019394399598240852;
                center[2] = 13.114355087280273 - 0.6431751847267151;

                position[0] = 1.6819782257080078;
                position[1] = 3.816258668899536;
                position[2] = 13.114355087280273;
              }
