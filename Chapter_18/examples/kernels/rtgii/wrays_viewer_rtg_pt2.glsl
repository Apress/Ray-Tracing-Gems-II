...
vec3 sampleEnvMap(vec3 dir)
{
  ...
  return 1.0*pow(texture(env_map, vec2(u, 1.0 - v)).rgb, vec3(2.2));
}
...
vec3 sampleEnvMapNoScale(vec3 dir)
{
  float u = 0.5 + atan(dir.z, dir.x) / (2.0 * RG_PI);
	float v = 0.5 - sin(dir.y) / RG_PI;
	return 0.5*pow(texture(env_map, vec2(u, 1.0 - v)).rgb, vec3(2.2));
}
...
vec3 computeDirectionalLightContribution(vec3 pos, vec3 geom_normal, out vec3 light_direction)
{
  ...
  light_direction = getDirectionFromPhiTheta(90.0, 70.0); // fireplace
  light_direction = getDirectionFromPhiTheta(60.0, 70.0); // dead nature2
  //light_direction = getDirectionFromPhiTheta(160.0, 70.0); // dead nature3
  light_direction = getDirectionFromPhiTheta(13.0, 80.0); // dead nature3
  const float light_intensity = 10.5*0.1;
...
vec3 computePointLightContributionFireplace(vec3 pos, vec3 geom_normal, out vec3 light_direction)
{
  vec3 light_positions[2] = vec3[2](vec3(-0.39, 1.5, 1.26), vec3(-1.2, 1.48, 1.26));
  ...
  int light_index = clamp(int(floor(randoms.x * 2.0)), 0, 1);
  
  vec3 light_color = vec3(light_intensity);

  vec3 light_pos = light_positions[light_index];
  float dist = length(light_pos - pos);
  dist = max(POSITION_OFFSET, dist - POSITION_OFFSET);

  vec3 dir = normalize(light_pos - pos);
  light_direction = dir;    

  float visible = 1.0 - float(wr_query_occlusion(0, pos + POSITION_OFFSET * geom_normal.rgb, dir, dist));
  
  float NdotL = max(0.0, dot(light_direction, geom_normal));
  directLight = visible * NdotL * light_intensity * light_color;

  return 0.1*directLight;
}
vec3 computePointLightContributionDeadNature3(vec3 pos, vec3 geom_normal, out vec3 light_direction)
{
  //vec3 light_position = vec3(74.89, 157.13, -14.81);
  //vec3 light_position = vec3(14.89, 157.13, -14.81);
  vec3 light_position = vec3(95, 176, 57);
      
  vec3 directLight = vec3(0);
  float light_intensity = 0.5;
   
  float dist = length(light_position - pos);
  dist = max(POSITION_OFFSET, dist - POSITION_OFFSET);
  ...
  float NdotL = max(0.0, dot(light_direction, geom_normal));
  directLight = visible * NdotL * light_intensity * vec3(1);
...
vec3 computeDirectLightContribution(vec3 pos, vec3 geom_normal, out vec3 light_direction)
{
  ...
  directLight *= vec3(250, 214, 165) / 255.0;
  directLight = computePointLightContributionDeadNature3(pos, geom_normal, light_direction); 
  //vec3 directLight = computeAreaLightQuadContribution2(pos, geom_normal, light_direction);
  //vec3 directLight = computePointLightContributionFireplace(pos, geom_normal, light_direction);  
...
void main() {
  ...
  vec3 light_direction = 0.0*getDirectionFromPhiTheta(290.0, 40.0);
