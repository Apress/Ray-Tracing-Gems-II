...
{
  addFace(mesh,0,-0.5f*(du+dv+dw),du,dv);
  addFace(mesh,1,-0.5f*(du+dv+dw),du,dw);
  addFace(mesh,2,-0.5f*(du+dv+dw),dv,dw);

  addFace(mesh,3,0.5f*(du+dv+dw),-du,-dv);
  addFace(mesh,4,0.5f*(du+dv+dw),-du,-dw);
  addFace(mesh,5,0.5f*(du+dv+dw),-dv,-dw);
}
                   ...
                   const vec3i coord)
{
  ...
  OWLBuffer indexBuffer
    = owlDeviceBufferCreate(context,OWL_INT4,mesh.indices.size(),mesh.indices.data());
  ...
  std::vector<OWLBuffer> textureSets;
  for (int textureSetID = 0; textureSetID < NUM_TEXTURE_SETS; textureSetID++) {
    std::vector<OWLTexture> textureSet;
    ...
    OWLBuffer textureSetBuffer
      = owlDeviceBufferCreate(context,
                              ...
                              textureSet.size(),
                              textureSet.data());
    textureSets.push_back(textureSetBuffer);
  }
  OWLBuffer textureSetsBuffer
    = owlDeviceBufferCreate(context,OWL_BUFFER,
                            textureSets.size(),
                            textureSets.data());
...
Viewer::Viewer()
{
  ...
  OWLVarDecl trianglesGeomVars[] = {
    ...
    { "textureSets",  OWL_BUFFER, OWL_OFFSETOF(TrianglesGeomData,textureSets)},
    { nullptr }
  };
  OWLGeomType trianglesGeomType
...
{
  LOG("owl.samples: example '" << av[0] << "' starting up");

  std::cout << "-------------------------------------------------------" << std::endl;
  std::cout << "This sample mostly exists to test and demonstrate the" << std::endl;
  std::cout << "'buffers of buffer' and 'buffers of textures' capability." << std::endl;
  std::cout << "In this example, each of the rotating cubes has N sets of" << std::endl;
  std::cout << "textures (with one texture per face per set)" << std::endl;
  std::cout << "that are stored in one OWLBuffer of OWLTextures per set," << std::endl;
  std::cout << "and then in one OWLBuffer of OWLBuffers for all sets;" << std::endl;
  std::cout << "the geom's CH program then picks from a different set" << std::endl;
  std::cout << "based on runtime (switching once every second)" << std::endl;
  std::cout << "-------------------------------------------------------" << std::endl;
  Viewer viewer;
