...
function wrays_viewer_init() {
  ...
  WebRaysViewer.shader_urls = [
                               ...
                               'kernels/screen_fill.vs.glsl', 'kernels/rtgii/wrays_viewer_rtg_reflections.glsl',
    ...
    Promise.all(responses.map(res => res.blob())).then(buffers => {
      ...
      Promise.all(decoded_imgs.map(img=>img.decode())).then(responses => {
        ...
        Promise.all(WebRaysViewer.shader_urls.map(url =>
          ...
          const gltf_file = 'gltf/deadNature4.gltf';
          ...
          webrays_load_gltf(gltf_file).then(res => {
            ...
            
            {
              ...
              let position = [center[0], maxValue[1] - 1.0, maxValue[2] + 1.0];
               ...
               if(gltf_file === 'gltf/deadNature4.gltf')
              {
                position[0] = 61.73;
                position[1] = 94.64;
                position[2] = 18.2;

                center[0] = position[0] - 0.8703;
                center[1] = position[1] - 0.3374;
                center[2] = position[2] - 0.35856;

                position[0] = -8.106;
                position[1] = 94.663;
                position[2] = 47.393;

                center[0] = position[0] + 0.0195;
                center[1] = position[1] - 0.22;
                center[2] = position[2] - 0.97522;

                position[0] = -8.1489;
                position[1] = 89.818;
                position[2] = 38.309;

                center[0] = position[0] - 0.008;
                center[1] = position[1] - 0.0022;
                center[2] = position[2] - 0.9999;
              }
...
function wrays_viewer_render_dr() {
  ...
  //console.log(WebRaysViewer.hybrid_buffer.FBO);
  gl.bindFramebuffer(gl.FRAMEBUFFER, WebRaysViewer.hybrid_buffer.FBO);
  ...
  for(let j = 0; j < mesh.primitives.length; j++)
  {
    ...
    if(primitive.indices !== null)
    {
      ...
      if(primitive.material && primitive.material.pbrMetallicRoughness)
      {
        ...
        type = primitive.material.alphaMode && primitive.material.alphaMode === 'BLEND' ? MaterialType.FRESNEL_SPECULAR : type;
        ...
        r_mesh.materials.push({
          ...
          nIndex: IOR_VALUES.n_index_glass,
...
function webrays_load_gltf(url) 
{
  ...
  const update = (resolve, reject) => {
    _gltfLoader.load_GLTF(url, glTF => {
      ...
      r_mesh.materialTextureURLs = r_mesh.materialTextureURLs.map(e => glTF.textures[e].source.currentSrc);
